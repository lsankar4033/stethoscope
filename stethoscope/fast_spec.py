# TODO: I don't need everything here!
...
def _inner_shuffle_list(input: Sequence[ValidatorIndex], seed: Root, dir: bool):
    ...
    while True:
        ...
        buf[_SHUFFLE_H_PIVOT_VIEW_SIZE:] = (
            (pivot >> 8) & 0xffff_ffff).to_bytes(length=4, byteorder=ENDIANNESS)
        source = hash(buf)
        ...
        while i < mirror:
            ...
            if j & 0xff == 0xff:
                ...
                buf[_SHUFFLE_H_PIVOT_VIEW_SIZE:] = (
                    (j >> 8) & 0xffff_ffff).to_bytes(length=4, byteorder=ENDIANNESS)
                source = hash(buf)
...
def compute_proposer_index(state: BeaconState, indices: Sequence[ValidatorIndex], seed: Bytes32) -> ValidatorIndex:
    ...
    while True:
        candidate_index = indices[compute_shuffled_index(
            ValidatorIndex(i % len(indices)), len(indices), seed)]
...
def get_seed(state: BeaconState, epoch: Epoch, domain_type: DomainType) -> Bytes32:
    ...
    """
    mix = get_randao_mix(state, Epoch(epoch + EPOCHS_PER_HISTORICAL_VECTOR -
                                      MIN_SEED_LOOKAHEAD - 1))  # Avoid underflow
...
def process_final_updates(epochs_ctx: EpochsContext, process: EpochProcess, state: BeaconState) -> None:
    ...
    for index, status in enumerate(process.statuses):
        ...
        if balance < effective_balance or effective_balance + 3 * HALF_INCREMENT < balance:
            new_effective_balance = min(balance - balance %
...
def slash_validator(epochs_ctx: EpochsContext, state: BeaconState,
    ...
    validator.withdrawable_epoch = max(validator.withdrawable_epoch,
